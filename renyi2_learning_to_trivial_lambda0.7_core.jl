"""
renyi2_learning_to_trivial_lambda0.7_core.jl

Shared, proposal-aligned physics core for the fixed-lambda=0.7 interior slice
...
`renyi2_right_boundary_core.jl`, so that every entry point for this slice
(cluster jobs via `run_learning_to_trivial_lambda0.7_scan.jl`) uses exactly
...
with delta = 0.7. The fixed interior slice studied here is lambda = 0.7, i.e.

    lambda_x = 0.49,  lambda_zz = 0.21,  q_x = q_zz = q in [0, 1/2].

Layer order per full time step (unlike the right edge, BOTH weak-measurement
layers are active here since lambda_zz != 0):
