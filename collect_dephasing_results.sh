...
# Script to collect and package dephasing job results for transfer
# Run this on the cluster after dephasing jobs complete

echo "=== Dephasing Job Results Collection ==="
echo "Started at: $(date)"

# Create results directory with timestamp
TIMESTAMP=$(date +"%Y%m%d_%H%M")
RESULTS_DIR="dephasing_results_${TIMESTAMP}"
mkdir -p $RESULTS_DIR

echo "Collecting results into: $RESULTS_DIR"

# Copy all dephasing job output files with correct naming pattern
echo "Searching for dephasing result files..."
echo "Looking for pattern: dephasing_L*_lam*_P*_s*.json"
...
for dir in $SEARCH_DIRS; do
    if [ -d "$dir" ]; then
        ...
        FILES_IN_DIR=$(find "$dir" -name "dephasing_L*_lam*_P*_s*.json" -type f 2>/dev/null)
...
if [ -z "$FOUND_FILES" ]; then
    echo "No files found with pattern dephasing_L*_lam*_P*_s*.json"
    echo "Searching for any dephasing*.json files in all directories..."
    for dir in $SEARCH_DIRS; do
        if [ -d "$dir" ]; then
            FILES_IN_DIR=$(find "$dir" -name "dephasing*.json" -type f 2>/dev/null)
            if [ ! -z "$FILES_IN_DIR" ]; then
                FOUND_FILES="$FOUND_FILES $FILES_IN_DIR"
                echo "  Found dephasing*.json files in $dir"
            fi
        fi
    done
fi
...
if [ -z "$FOUND_FILES" ]; then
    echo "No dephasing result files found at all!"
    echo ""
...
cp jobs/params_dephasing.txt "$RESULTS_DIR/" 2>/dev/null || echo "  Warning: params_dephasing.txt not found"
cp jobs/dephasing_jobs.submit "$RESULTS_DIR/" 2>/dev/null || echo "  Warning: dephasing_jobs.submit not found"

# Copy a few sample log files for debugging if needed
echo "Copying sample log files..."
mkdir -p "$RESULTS_DIR/sample_logs"
find logs -name "*dephasing*.out" -type f | head -5 | while read file; do
    if [ -f "$file" ]; then
...
find logs -name "*dephasing*.err" -type f -size +0 | head -5 | while read file; do
    if [ -f "$file" ]; then
        cp "$file" "$RESULTS_DIR/sample_logs/"
    fi
done
...
Dephasing Job Results Collection Summary
=====================================
...
JSON Results:
$(find "$RESULTS_DIR" -name "dephasing*.json" | wc -l) dephasing result files
...
Collected: $(find "$RESULTS_DIR" -name "dephasing*.json" | wc -l) result files
