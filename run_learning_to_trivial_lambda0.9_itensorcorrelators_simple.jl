...
Scan q = P_x = P_zz at fixed λ_x = 0.63, λ_zz = 0.07 (delta=0.7, lambda=0.9).
...
point `run_learning_to_trivial_lambda0.9_itensorcorrelators_scan.jl`.
"""
...
L = 8                    # System size
lambda_x = 0.63         # X measurement strength (FIXED)
lambda_zz = 0.07        # ZZ measurement strength (FIXED)
...
seed = 42               # Random seed
output_dir = "learning_to_trivial_lambda0.9_itensorcorrelators_results"
# =============================

@assert isapprox(lambda_x, 0.63; atol=1e-9) "Proposal-aligned lambda=0.9 slice requires lambda_x = 0.63."
@assert isapprox(lambda_zz, 0.07; atol=1e-9) "Proposal-aligned lambda=0.9 slice requires lambda_zz = 0.07."
...
for (i, q) in enumerate(P_values)
    ...
    output_file = joinpath(output_dir, "learning_to_trivial_lambda0.9_itensorcorrelators_L$(L)_$(timestamp).json")
