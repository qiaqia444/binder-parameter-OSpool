...
# Collect and organize right boundary scan results (ITensorCorrelators.jl variant)
# Run this script after all HTCondor jobs complete

echo "=== Right Boundary Scan Results Collection (ITensorCorrelators.jl) ==="
echo "Starting collection at: $(date)"

# Create timestamped results directory
TIMESTAMP=$(date +%Y%m%d_%H%M)
RESULTS_DIR="right_boundary_itensorcorrelators_results_${TIMESTAMP}"
...
for L in 8 16 24 32; do
    ...
    find output -name "right_boundary_itensorcorrelators_L${L}_lx0.70_lzz0.00_*.json" ! -name "*FAILED*" -exec cp {} "../${RESULTS_DIR}/L${L}/" \; 2>/dev/null
...
# Check for failures
cd jobs
failure_count=$(ls output/right_boundary_itensorcorrelators_*_FAILED.json 2>/dev/null | wc -l)
...
echo "  julia analyze_right_boundary_itensorcorrelators.jl"
echo ""
