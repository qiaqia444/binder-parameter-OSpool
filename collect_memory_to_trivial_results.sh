...
# Collect and organize memory-to-trivial transition scan results
# Run this script after all HTCondor jobs complete

echo "=== Memory-to-Trivial Transition Scan Results Collection ==="
echo "Starting collection at: $(date)"

# Create timestamped results directory
TIMESTAMP=$(date +%Y%m%d_%H%M)
RESULTS_DIR="memory_to_trivial_results_${TIMESTAMP}"
...
# Organize by system size (collect ALL lx0.10 lzz0.70 files, no time filter)
echo "Collecting ALL λ_x=0.1, λ_zz=0.7 files..."
for L in 8 10 12 14 16; do
    # Collect all files matching pattern (no -mtime filter)
    find output -name "memory_to_trivial_L${L}_lx0.10_lzz0.70_*.json" ! -name "*FAILED*" -exec cp {} "../${RESULTS_DIR}/L${L}/" \; 2>/dev/null
