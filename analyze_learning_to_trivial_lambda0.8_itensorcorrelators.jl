...
"""
Analyze Learning-to-Trivial Transition Results
(ITensorCorrelators.jl variant, lambda=0.8 family)

Parameters: λ_x = 0.56, λ_zz = 0.14
Rényi-2 Binder moments computed via ITensorCorrelators.jl n-point correlators.
"""
...
# Configuration
# ====================================================================
const DEFAULT_RESULTS_DIR = "learning_to_trivial_lambda0.8_itensorcorrelators_results"
...
function resolve_results_dir(results_dir::String=DEFAULT_RESULTS_DIR)
    ...
    candidates = filter(
        name -> startswith(name, "learning_to_trivial_lambda0.8_itensorcorrelators_results_") && isdir(name),
        readdir(pwd())
    )
...
function print_analysis(df::DataFrame)
    ...
    println("="^80)
    println("LEARNING-TO-TRIVIAL TRANSITION ANALYSIS (ITensorCorrelators.jl)")
    println("lambda=0.8 family: λ_x = 0.56, λ_zz = 0.14")
    println("="^80)
    println()
...
function save_plot(summary_df::DataFrame)
    ...
    p = plot(
        ...
        title = L"\lambda_x = 0.56, \lambda_{zz} = 0.14" * " (ITensorCorrelators.jl)",
    ...
    png_file = "learning_to_trivial_lambda0.8_itensorcorrelators_binder_vs_px.png"
    pdf_file = "learning_to_trivial_lambda0.8_itensorcorrelators_binder_vs_px.pdf"
    savefig(p, png_file)
    savefig(p, pdf_file)
...
function main()
    ...
    summary_file = "learning_to_trivial_lambda0.8_itensorcorrelators_analysis_summary.csv"
