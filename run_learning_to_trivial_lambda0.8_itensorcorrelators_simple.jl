...
"""
Learning-to-Trivial Scan (ITensorCorrelators.jl variant) - local convenience
script

Scan q = P_x = P_zz at fixed λ_x = 0.56, λ_zz = 0.14 (delta=0.7, lambda=0.8).
Observable: trajectory-averaged replica-overlap Rényi-2 Binder, with the
Rényi-2 moments computed via ITensorCorrelators.jl n-point correlators (see
`renyi2_learning_to_trivial_lambda0.8_itensorcorrelators_core.jl`).

Physics is the doubled-MPS Born-sampling model validated in
`lambda_0p7_renyi2_proposal_aligned_v3.ipynb`, shared with the cluster entry
point `run_learning_to_trivial_lambda0.8_itensorcorrelators_scan.jl`.
"""
...
L = 8                    # System size
lambda_x = 0.56         # X measurement strength (FIXED)
lambda_zz = 0.14        # ZZ measurement strength (FIXED)
...
seed = 42               # Random seed
output_dir = "learning_to_trivial_lambda0.8_itensorcorrelators_results"
# =============================

@assert isapprox(lambda_x, 0.56; atol=1e-9) "Proposal-aligned lambda=0.8 slice requires lambda_x = 0.56."
@assert isapprox(lambda_zz, 0.14; atol=1e-9) "Proposal-aligned lambda=0.8 slice requires lambda_zz = 0.14."

println("="^70)
println("LEARNING-TO-TRIVIAL SCAN (ITensorCorrelators.jl): Rényi-2 Binder Analysis")
println("="^70)
...
println("  λ_zz = $lambda_zz (ZZ measurement strength - FIXED)")
...
for (i, q) in enumerate(P_values)
    ...
    output_file = joinpath(output_dir, "learning_to_trivial_lambda0.8_itensorcorrelators_L$(L)_$(timestamp).json")
