...
`right_boundary_renyi2_proposal_aligned_v4.ipynb` and implemented once, shared
by all right-boundary entry points, in `renyi2_right_boundary_core.jl`.
...
function parse_commandline()
    s = ArgParseSettings(description = "Right boundary scan: vary q=P_x=P_zz at fixed λ_x with proposal-aligned Rényi-2 Binder")
