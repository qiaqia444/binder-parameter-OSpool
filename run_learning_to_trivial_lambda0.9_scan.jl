...
"""
Learning-to-Trivial Scan: Fixed lambda=0.9 interior slice with Rényi-2 Binder
...
all entry points for this slice, in `renyi2_learning_to_trivial_lambda0.9_core.jl`.
...
include("renyi2_learning_to_trivial_lambda0.9_core.jl")

function parse_commandline()
    s = ArgParseSettings(description = "Learning-to-trivial scan: vary q=P_x=P_zz at fixed λ_x=0.63, λ_zz=0.07 with proposal-aligned Rényi-2 Binder")
    ...
    @add_arg_table! s begin
        ...
        "--output_dir"
            ...
            arg_type = String
            default = "learning_to_trivial_lambda0.9_results"
...
function main()
    ...
    println("="^70)
    println("LEARNING-TO-TRIVIAL SCAN: Fixed lambda=0.9 interior slice")
    println("="^70)
    ...
    for (i, q) in enumerate(q_values)
        ...
        # Store results (field names kept compatible with the learning_to_trivial_lambda0.7 pipeline)
        result_dict = Dict(
            "L" => L,
            "lambda_x" => lambda_x,
            "lambda_zz" => lambda_zz,
        ...
        if !isempty(output_file)
            ...
            outpath = joinpath(output_dir, "learning_to_trivial_lambda0.9_L$(L)_$(timestamp).json")
        end
    ...
    if isempty(output_file)
        ...
        final_output = joinpath(output_dir, "learning_to_trivial_lambda0.9_L$(L)_$(timestamp)_final.json")
