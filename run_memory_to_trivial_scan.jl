...
"""
Memory-to-Trivial Transition Scan: Fixed Measurement Strengths

Scan dephasing probability P_x = P_zz from 0 to 0.5 at FIXED measurement strengths.
This explores the transition between memory-keeping and trivial phases.
...
Parameters:
- λ_x = 0.1 (X measurement strength, FIXED)
- λ_zz = 0.7 (ZZ measurement strength, FIXED)
- Scan P_x = P_zz from 0 to 0.5
"""
...
function parse_commandline()
    s = ArgParseSettings(description = "Memory-to-trivial scan: vary P at fixed λ")
    ...
    @add_arg_table! s begin
        ...
        "--output_dir"
            ...
            arg_type = String
            default = "memory_to_trivial_results"
...
function main()
    ...
    println("="^70)
    println("MEMORY-TO-TRIVIAL TRANSITION SCAN")
    println("="^70)
    ...
    for (i, P) in enumerate(P_values)
        ...
        if !isempty(output_file)
            ...
            outpath = joinpath(output_dir, "memory_to_trivial_L$(L)_lambda$(lambda_x)_$(timestamp).json")
        end
    ...
    if isempty(output_file)
        ...
        final_output = joinpath(output_dir, "memory_to_trivial_L$(L)_lambda$(lambda_x)_$(timestamp)_final.json")
