...
Fixed: λ_x = 0.56 (X measurements), λ_zz = 0.14 (ZZ measurements)
...
# Fixed measurement strengths (lambda=0.8 family)
lambda_x = 0.56   # X measurement strength
lambda_zz = 0.14  # ZZ measurement strength
...
# Open output file
open("params_learning_to_trivial_lambda0.8_itensorcorrelators.txt", "w") do f
    seed = seed_start
    ...
    for L in L_values
        for P in P_values
            for sample in 1:n_samples
                ...
                out_prefix = @sprintf("learning_to_trivial_lambda0.8_itensorcorrelators_L%d_lx%.2f_lzz%.2f_Px%.2f_s%d",
...
println("Parameter file generated: params_learning_to_trivial_lambda0.8_itensorcorrelators.txt")
