...
"""
Generate parameter file for Dephasing P=0.5 jobs (L=8,12,16) - CORRECTED VERSION
Increased parallelization for faster completion
"""

using Printf

# System sizes for small systems (including L=20)
L_values = [8, 12, 16, 20]
...
# Starting seed for small dephasing (use 5000+ range)
seed_start = 5001

# Open output file
open("params_dephasing_p05.txt", "w") do f
    seed = seed_start
    ...
    for L in L_values
        for lambda in lambda_values
            ...
            for sample in 1:n_samples
                out_prefix = @sprintf("dephasing_p05_L%d_lam%.2f_P%.1f_s%d", L, lambda, P_x, sample)
