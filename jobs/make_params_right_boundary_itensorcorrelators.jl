...
"""
Generate parameter file for Right Boundary Scan (ITensorCorrelators.jl variant)
Explores measurement-induced phase transition with Rényi-2 Binder, moments
computed via ITensorCorrelators.jl n-point correlators instead of a global
replica-overlap MPO.
...
# Open output file
open("params_right_boundary_itensorcorrelators.txt", "w") do f
    seed = seed_start
    ...
    for L in L_values
        for P in P_values
            for sample in 1:n_samples
                ...
                out_prefix = @sprintf("right_boundary_itensorcorrelators_L%d_lx%.2f_lzz%.2f_Px%.2f_s%d", 
...
println("Parameter file generated: params_right_boundary_itensorcorrelators.txt")
