...
# HTCondor job script for standard Binder parameter calculation
# This script runs standard quantum trajectory simulations

# Get arguments from HTCondor
ARGS="$@"

echo "=== Standard Binder Parameter Job Start ==="
...
if command -v apptainer >/dev/null 2>&1; then
    ...
    # Run simulation
    echo "Running standard simulation in container..."
    echo "Command: apptainer exec julia.sif julia --project=. run.jl $ARGS"
    apptainer exec julia.sif julia --project=. run.jl $ARGS
    EXIT_CODE=$?
    ...
    # Run simulation
    echo "Running standard simulation in container..."
    echo "Command: singularity exec julia.sif julia --project=. run.jl $ARGS"
    singularity exec julia.sif julia --project=. run.jl $ARGS
    EXIT_CODE=$?
else
...
echo "=== Standard Binder Parameter Job End ==="
