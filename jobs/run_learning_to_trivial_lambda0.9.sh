...
# HTCondor job script for learning-to-trivial scan (fixed lambda=0.9 slice:
# λ_x = 0.63, λ_zz = 0.07)
...
echo "=== Learning-to-Trivial Scan Job Start (λ_x = 0.63, λ_zz = 0.07) ==="
...
echo "Command: julia --project=. run_learning_to_trivial_lambda0.9_scan.jl --L $L --lambda_x $lambda_x --lambda_zz $lambda_zz --P_min $P_x --P_max $P_x --P_steps 1 --ntrials $ntrials --seed $seed --output_dir output --output_file ${out_prefix}.json"

julia --project=. run_learning_to_trivial_lambda0.9_scan.jl \
    --L $L \
    --lambda_x $lambda_x \
    --lambda_zz $lambda_zz \
    --P_min $P_x \
