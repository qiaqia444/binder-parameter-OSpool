...
# Script to collect results from completed P=0.2 dephasing jobs (CORRECTED VERSION)
# Run this on the submit node after jobs complete
# L=8,12,16,20 (small systems)

echo "Collecting P=0.2 dephasing simulation results (CORRECTED)..."
...
# Count total jobs
total_jobs=$(wc -l < params_dephasing_p02.txt)
echo "Total jobs submitted: $total_jobs (expected 680)"

# Find and copy result files
echo "Searching for dephasing_p02_*.json files..."
find . -name "dephasing_p02_L8_*.json" -o -name "dephasing_p02_L12_*.json" -o -name "dephasing_p02_L16_*.json" -o -name "dephasing_p02_L20_*.json" | while read file; do cp "$file" ${RESULTS_DIR}/; done
...
if [ ${NUM_FILES} -eq 0 ]; then
    ...
    for L in 8 12 16 20; do
        count=$(ls -1 ${RESULTS_DIR}/dephasing_p02_L${L}_*.json 2>/dev/null | wc -l)
