...
# variant, fixed lambda=0.8 slice: λ_x = 0.56, λ_zz = 0.14)
...
echo "=== Learning-to-Trivial Scan Job Start (ITensorCorrelators.jl, λ_x = 0.56, λ_zz = 0.14) ==="
...
echo "Command: julia --project=. run_learning_to_trivial_lambda0.8_itensorcorrelators_scan.jl --L $L --lambda_x $lambda_x --lambda_zz $lambda_zz --P_min $P_x --P_max $P_x --P_steps 1 --ntrials $ntrials --seed $seed --output_dir output --output_file ${out_prefix}.json"

julia --project=. run_learning_to_trivial_lambda0.8_itensorcorrelators_scan.jl \
    --L $L \
    --lambda_x $lambda_x \
    --lambda_zz $lambda_zz \
    --P_min $P_x \
