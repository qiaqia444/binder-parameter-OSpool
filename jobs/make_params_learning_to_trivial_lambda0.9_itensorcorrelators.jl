...
"""
Generate parameter file for Learning-to-Trivial Transition Scan
(ITensorCorrelators.jl variant, λ_x = 0.63, λ_zz = 0.07)
Explores the phase transition from learning to trivial phase with Rényi-2
Binder, moments computed via ITensorCorrelators.jl n-point correlators.

Scans: P_x = P_zz from 0 to 0.5 (coupled dephasing)
Fixed: λ_x = 0.63 (X measurements), λ_zz = 0.07 (ZZ measurements)
(delta = 0.7, lambda = 0.9)
"""
...
# Fixed measurement strengths (lambda=0.9 family)
lambda_x = 0.63   # X measurement strength
lambda_zz = 0.07  # ZZ measurement strength
...
# Open output file
open("params_learning_to_trivial_lambda0.9_itensorcorrelators.txt", "w") do f
    seed = seed_start
    ...
    for L in L_values
        for P in P_values
            for sample in 1:n_samples
                ...
                out_prefix = @sprintf("learning_to_trivial_lambda0.9_itensorcorrelators_L%d_lx%.2f_lzz%.2f_Px%.2f_s%d",
...
println("Parameter file generated: params_learning_to_trivial_lambda0.9_itensorcorrelators.txt")
