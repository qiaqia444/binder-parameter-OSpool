...
# Script to collect results from completed P=0.8 dephasing jobs
# Run this on the submit node after jobs complete

echo "Collecting P=0.8 dephasing simulation results..."

# Create results directory
mkdir -p results_dephasing_p08

# Count total jobs
total_jobs=$(wc -l < params_dephasing_p08.txt)
echo "Total jobs submitted: $total_jobs"

# Collect all completed result files
count=0
for dir in dephasing_p08_L*_P0.8_s*/; do
    if [ -d "$dir" ]; then
        if [ -d "$dir/output" ]; then
            # Copy all JSON files from output directory
            for file in "$dir"/output/*.json; do
                if [ -f "$file" ]; then
                    cp "$file" results_dephasing_p08/
                    ((count++))
                fi
            done
        fi
...
echo "Results saved in: results_dephasing_p08/"

# List some examples
echo ""
echo "Sample of collected files:"
ls results_dephasing_p08/ | head -10
