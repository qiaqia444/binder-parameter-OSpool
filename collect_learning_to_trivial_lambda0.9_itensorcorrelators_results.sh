...
# (ITensorCorrelators.jl variant, λ_x = 0.63, λ_zz = 0.07)
# Run this script after all HTCondor jobs complete

echo "=== Learning-to-Trivial Transition Scan Results Collection (ITensorCorrelators.jl, λ_x = 0.63, λ_zz = 0.07) ==="
echo "Starting collection at: $(date)"

# Create timestamped results directory
TIMESTAMP=$(date +%Y%m%d_%H%M)
RESULTS_DIR="learning_to_trivial_lambda0.9_itensorcorrelators_results_${TIMESTAMP}"
...
EXPECTED_LAMBDA_X="0.63"
EXPECTED_LAMBDA_ZZ="0.07"
SAMPLE_FILE=$(ls output/learning_to_trivial_lambda0.9_itensorcorrelators_L*_lx0.63_lzz0.07_*.json 2>/dev/null | head -1)

if [ -z "$SAMPLE_FILE" ]; then
    echo "ERROR: No files found matching pattern learning_to_trivial_lambda0.9_itensorcorrelators_L*_lx0.63_lzz0.07_*.json"
...
# Organize by system size (collect ALL lx0.63 lzz0.07 files)
...
for L in 8 16 24 32; do
    ...
    find output -name "learning_to_trivial_lambda0.9_itensorcorrelators_L${L}_lx0.63_lzz0.07_*.json" ! -name "*FAILED*" -exec cp {} "../${RESULTS_DIR}/L${L}/" \; 2>/dev/null
...
# Check for failures
cd jobs
failure_count=$(ls output/learning_to_trivial_lambda0.9_itensorcorrelators_*_FAILED.json 2>/dev/null | wc -l)
