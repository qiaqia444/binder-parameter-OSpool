...
# Script to collect and package dephasing P=0.5 job results for transfer
# Run this on the cluster after dephasing P=0.5 jobs complete

echo "=== Dephasing P=0.5 Job Results Collection ==="
...
# Copy all dephasing P=0.5 job output files with correct naming pattern
echo "Searching for dephasing P=0.5 result files..."
echo "Looking for pattern: dephasing_p05_L*_lam*_P*_s*.json"
...
for dir in $SEARCH_DIRS; do
    if [ -d "$dir" ]; then
        ...
        FILES_IN_DIR=$(find "$dir" -name "dephasing_p05_L*_lam*_P*_s*.json" -type f 2>/dev/null)
...
if [ -z "$FOUND_FILES" ]; then
    echo "No files found with pattern dephasing_p05_L*_lam*_P*_s*.json"
    echo "Searching for any dephasing_p05*.json files in all directories..."
    for dir in $SEARCH_DIRS; do
        if [ -d "$dir" ]; then
            FILES_IN_DIR=$(find "$dir" -name "dephasing_p05*.json" -type f 2>/dev/null)
            if [ ! -z "$FILES_IN_DIR" ]; then
                FOUND_FILES="$FOUND_FILES $FILES_IN_DIR"
                echo "  Found dephasing_p05*.json files in $dir"
            fi
        fi
    done
fi
...
if [ -z "$FOUND_FILES" ]; then
    echo "No dephasing P=0.5 result files found at all!"
    echo ""
...
cp jobs/params_dephasing_p05.txt "$RESULTS_DIR/" 2>/dev/null || echo "  Warning: params_dephasing_p05.txt not found"
cp jobs/dephasing_p05_jobs.submit "$RESULTS_DIR/" 2>/dev/null || echo "  Warning: dephasing_p05_jobs.submit not found"

# Copy a few sample log files for debugging if needed
echo "Copying sample log files..."
mkdir -p "$RESULTS_DIR/sample_logs"
find logs -name "*dephasing_p05*.out" -type f | head -5 | while read file; do
    if [ -f "$file" ]; then
...
find logs -name "*dephasing_p05*.err" -type f -size +0 | head -5 | while read file; do
    if [ -f "$file" ]; then
        cp "$file" "$RESULTS_DIR/sample_logs/"
    fi
done
...
Dephasing P=0.5 Job Results Collection Summary
=====================================
...
JSON Results:
$(find "$RESULTS_DIR" -name "dephasing_p05*.json" | wc -l) dephasing P=0.5 result files
...
Collected: $(find "$RESULTS_DIR" -name "dephasing_p05*.json" | wc -l) result files
