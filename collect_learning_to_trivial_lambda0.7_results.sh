...
# Collect and organize learning-to-trivial transition scan results (λ_x = 0.49, λ_zz = 0.21)
# Run this script after all HTCondor jobs complete

echo "=== Learning-to-Trivial Transition Scan Results Collection (λ_x = 0.49, λ_zz = 0.21) ==="
echo "Starting collection at: $(date)"

# Create timestamped results directory
TIMESTAMP=$(date +%Y%m%d_%H%M)
RESULTS_DIR="learning_to_trivial_lambda0.7_results_${TIMESTAMP}"
...
EXPECTED_LAMBDA_X="0.49"
EXPECTED_LAMBDA_ZZ="0.21"
SAMPLE_FILE=$(ls output/learning_to_trivial_lambda0.7_L*_lx0.49_lzz0.21_*.json 2>/dev/null | head -1)

if [ -z "$SAMPLE_FILE" ]; then
    echo "ERROR: No files found matching pattern learning_to_trivial_lambda0.7_L*_lx0.49_lzz0.21_*.json"
...
# Organize by system size (collect ALL lx0.49 lzz0.21 files)
...
for L in 8 10 12 14 16; do
    ...
    find output -name "learning_to_trivial_lambda0.7_L${L}_lx0.49_lzz0.21_*.json" ! -name "*FAILED*" -exec cp {} "../${RESULTS_DIR}/L${L}/" \; 2>/dev/null
...
# Check for failures
cd jobs
failure_count=$(ls output/learning_to_trivial_lambda0.7_*_FAILED.json 2>/dev/null | wc -l)
