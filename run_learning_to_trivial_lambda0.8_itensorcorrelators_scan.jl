...
"""
Learning-to-Trivial Scan (ITensorCorrelators.jl variant): Fixed lambda=0.8
interior slice with Rényi-2 Binder

Scan dephasing strength q = P_x = P_zz from 0 to 1/2 at FIXED measurement
strengths λ_x = 0.56, λ_zz = 0.14 (i.e. δ=0.7, λ=0.8). Both weak-measurement
layers (X and ZZ) are active here.

Physics mirrors `run_learning_to_trivial_lambda0.9_itensorcorrelators_scan.jl`
/ `renyi2_learning_to_trivial_lambda0.8_itensorcorrelators_core.jl` (the
proposal-aligned doubled-MPS Born-sampling model validated in
`lambda_0p7_renyi2_proposal_aligned_v3.ipynb`), with the Rényi-2 moments
(M2, M4) computed via ITensorCorrelators.jl n-point correlators instead of
applying a global replica-overlap MPO.
...
include("renyi2_learning_to_trivial_lambda0.8_itensorcorrelators_core.jl")

function parse_commandline()
    s = ArgParseSettings(description = "Learning-to-trivial scan (ITensorCorrelators.jl): vary q=P_x=P_zz at fixed λ_x=0.56, λ_zz=0.14 with proposal-aligned Rényi-2 Binder")
    ...
    @add_arg_table! s begin
        ...
        "--lambda_x"
            help = "X measurement strength (FIXED, must be 0.56 for this slice)"
            arg_type = Float64
            default = 0.56

        "--lambda_zz"
            help = "ZZ measurement strength (FIXED, must be 0.14 for this slice)"
            arg_type = Float64
            default = 0.14
        ...
        "--output_dir"
            ...
            arg_type = String
            default = "learning_to_trivial_lambda0.8_itensorcorrelators_results"
...
function main()
    ...
    @assert isapprox(lambda_x, 0.56; atol=1e-9) "Proposal-aligned lambda=0.8 slice requires lambda_x = 0.56 (delta=0.7, lambda=0.8)."
    @assert isapprox(lambda_zz, 0.14; atol=1e-9) "Proposal-aligned lambda=0.8 slice requires lambda_zz = 0.14 (delta=0.7, lambda=0.8)."

    println("="^70)
    println("LEARNING-TO-TRIVIAL SCAN (ITensorCorrelators.jl): Fixed lambda=0.8 interior slice")
    println("="^70)
    println("Physics: lambda=0.8, delta=0.7 => lambda_x=0.56, lambda_zz=0.14, q_x=q_zz=q")
    println("Method: doubled-MPS Born sampling (proposal-aligned), both weak layers active")
    println("Observable: trajectory-averaged replica-overlap Rényi-2 Binder (ITensorCorrelators.jl)")
    println()
    println("Parameters:")
    ...
    for (i, q) in enumerate(q_values)
        ...
        if !isempty(output_file)
            ...
            outpath = joinpath(output_dir, "learning_to_trivial_lambda0.8_itensorcorrelators_L$(L)_$(timestamp).json")
        end
