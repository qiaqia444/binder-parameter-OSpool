...
"""
Learning-to-Trivial Scan (ITensorCorrelators.jl variant): Fixed lambda=0.9
interior slice with Rényi-2 Binder

Scan dephasing strength q = P_x = P_zz from 0 to 1/2 at FIXED measurement
strengths λ_x = 0.63, λ_zz = 0.07 (i.e. δ=0.7, λ=0.9). Both weak-measurement
layers (X and ZZ) are active here.

Physics is identical to `run_learning_to_trivial_lambda0.9_scan.jl` /
`renyi2_learning_to_trivial_lambda0.9_core.jl` (the proposal-aligned
doubled-MPS Born-sampling model validated in
`lambda_0p7_renyi2_proposal_aligned_v3.ipynb`), but the Rényi-2 moments
(M2, M4) are computed with ITensorCorrelators.jl n-point correlators instead
of applying a global replica-overlap MPO -- see
...
include("renyi2_learning_to_trivial_lambda0.9_itensorcorrelators_core.jl")

function parse_commandline()
    s = ArgParseSettings(description = "Learning-to-trivial scan (ITensorCorrelators.jl): vary q=P_x=P_zz at fixed λ_x=0.63, λ_zz=0.07 with proposal-aligned Rényi-2 Binder")
    ...
    @add_arg_table! s begin
        ...
        "--lambda_x"
            help = "X measurement strength (FIXED, must be 0.63 for this slice)"
            arg_type = Float64
            default = 0.63

        "--lambda_zz"
            help = "ZZ measurement strength (FIXED, must be 0.07 for this slice)"
            arg_type = Float64
            default = 0.07
        ...
        "--output_dir"
            ...
            arg_type = String
            default = "learning_to_trivial_lambda0.9_itensorcorrelators_results"
...
function main()
    ...
    @assert isapprox(lambda_x, 0.63; atol=1e-9) "Proposal-aligned lambda=0.9 slice requires lambda_x = 0.63 (delta=0.7, lambda=0.9)."
    @assert isapprox(lambda_zz, 0.07; atol=1e-9) "Proposal-aligned lambda=0.9 slice requires lambda_zz = 0.07 (delta=0.7, lambda=0.9)."

    println("="^70)
    println("LEARNING-TO-TRIVIAL SCAN (ITensorCorrelators.jl): Fixed lambda=0.9 interior slice")
    println("="^70)
    println("Physics: lambda=0.9, delta=0.7 => lambda_x=0.63, lambda_zz=0.07, q_x=q_zz=q")
    ...
    for (i, q) in enumerate(q_values)
        ...
        if !isempty(output_file)
            ...
            outpath = joinpath(output_dir, "learning_to_trivial_lambda0.9_itensorcorrelators_L$(L)_$(timestamp).json")
        end
