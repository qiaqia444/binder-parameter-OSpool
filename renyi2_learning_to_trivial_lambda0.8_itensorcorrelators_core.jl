...
Shared, proposal-aligned physics core for the fixed-lambda=0.8 interior slice
("learning-to-trivial" transition scan), using ITensorCorrelators.jl for the
Rényi-2 moments. This mirrors
`renyi2_learning_to_trivial_lambda0.9_itensorcorrelators_core.jl` (itself
built from `renyi2_learning_to_trivial_lambda0.9_core.jl`, a direct
translation of the validated notebook

    lambda_0p7_renyi2_proposal_aligned_v3.ipynb

) with the Rényi-2 Binder moments (M2, M4) computed via ITensorCorrelators.jl
n-point correlators instead of applying a global replica-overlap MPO.

Exact phase-diagram parameterization:

    lambda_x = delta * lambda,  lambda_zz = delta * (1 - lambda),  q_x = q_zz = q

with delta = 0.7. The fixed interior slice studied here is lambda = 0.8, i.e.

    lambda_x = 0.56,  lambda_zz = 0.14,  q_x = q_zz = q in [0, 1/2].
