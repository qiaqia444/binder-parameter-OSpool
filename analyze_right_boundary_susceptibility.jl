...
"""
Analyze Right Boundary Susceptibility Scan Results (ITensorCorrelators.jl
variant) - Plot Generation Only

Load all JSON files, compute statistics, and generate plots.
Properly groups by (L, lambda_x, lambda_zz, P_x) to avoid mixing different
parameter sets.

Focus: Rényi-2 replica-overlap susceptibility chi2 = <Q^2>/L, computed via
...
function compute_statistics(df)
    """Compute mean and std of chi2 for each (L, lambda_x, lambda_zz, P_x) combination."""
    gdf = groupby(df, [:L, :lambda_x, :lambda_zz, :P_x])

    stats = combine(gdf) do group
        (
            chi2_mean = mean(group.chi2),
            chi2_std = std(group.chi2),
            chi2_sem = std(group.chi2) / sqrt(nrow(group)),
...
function plot_susceptibility_vs_px(stats)
    """Create plot of Rényi-2 overlap susceptibility vs P for λ_x=0.7, λ_zz=0."""

    p1 = plot(xlabel=L"P_{x} = P_{zz}", ylabel=L"\chi_2 = \langle Q^2 \rangle / L",
    ...
    for (idx, L) in enumerate(sort(unique(stats.L)))
        ...
        plot!(p1, L_data.P_x, L_data.chi2_mean,
              yerr=L_data.chi2_sem,
              label="L = $L",
              color=colors[idx],
              marker=markers[idx],
              markersize=6,
    ...
    savefig(p1, "right_boundary_susceptibility_lx0.7_lzz0.0_chi2_vs_px.pdf")
    savefig(p1, "right_boundary_susceptibility_lx0.7_lzz0.0_chi2_vs_px.png")
end

function plot_moments_vs_px(stats)
    """Create plot of M2 moment vs P_x."""
    ...
    savefig(p2, "right_boundary_susceptibility_lx0.7_lzz0.0_M2_vs_px.pdf")
    savefig(p2, "right_boundary_susceptibility_lx0.7_lzz0.0_M2_vs_px.png")
end

function main()
    # Find the most recent results directory
    results_dirs = filter(d -> startswith(d, "right_boundary_susceptibility_results_") && isdir(d), readdir())
    if isempty(results_dirs)
        println("ERROR: No results directory found matching 'right_boundary_susceptibility_results_*'")
        return
    end
    ...
    println("\nGenerating plots...")
    plot_susceptibility_vs_px(stats)
    plot_moments_vs_px(stats)

    println("✓ Plots generated:")
    println("  - right_boundary_susceptibility_lx0.7_lzz0.0_chi2_vs_px.pdf")
    println("  - right_boundary_susceptibility_lx0.7_lzz0.0_M2_vs_px.pdf")
end
