...
function plot_binder_vs_px(stats)
    """Create plot of Binder parameter vs P for λ_x=0.3, λ_zz=0.7."""
    
    # Should only have one parameter set now (lambda_x=0.3, lambda_zz=0.7)
    p1 = plot(xlabel=L"P_x = P_{zz}" * " (dephasing probability)", ylabel="Binder Parameter", 
              title=L"\lambda_x = 0.3, \lambda_{zz} = 0.7",
    ...
    savefig(p1, "memory_to_trivial_lx0.3_lzz0.7_binder_vs_p.pdf")
    savefig(p1, "memory_to_trivial_lx0.3_lzz0.7_binder_vs_p.png")
end
...
function plot_binder_by_L(stats)
    ...
    for (idx, L) in enumerate(sort(unique(stats.L)))
        ...
        p = plot(xlabel=L"P_x = P_{zz}" * " (dephasing probability)", 
                ...
                title=L"\lambda_x = 0.3, \lambda_{zz} = 0.7, $L = " * "$L",
        ...
        savefig(p, "memory_to_trivial_lx0.3_L$(L)_binder_vs_p.pdf")
        savefig(p, "memory_to_trivial_lx0.3_L$(L)_binder_vs_p.png")
    end
end
...
function print_summary(stats)
    ...
    println("  - memory_to_trivial_lx0.3_lzz0.7_binder_vs_p.{pdf,png}")
    println("  - memory_to_trivial_lx0.3_L{L}_binder_vs_p.{pdf,png} (for each L)")
    println("="^80 * "\n")
end

function main()
    # Find the most recent results directory
    results_dirs = filter(d -> startswith(d, "memory_to_trivial_lx0.3_results_") && isdir(d), readdir())
    if isempty(results_dirs)
        println("ERROR: No results directory found matching 'memory_to_trivial_lx0.3_results_*'")
        return
    end
