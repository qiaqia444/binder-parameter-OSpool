...
(ITensorCorrelators.jl variant, lambda=0.9 family)
...
# Configuration
# ====================================================================
const DEFAULT_RESULTS_DIR = "learning_to_trivial_lambda0.9_itensorcorrelators_results"
...
function resolve_results_dir(results_dir::String=DEFAULT_RESULTS_DIR)
    ...
    candidates = filter(
        name -> startswith(name, "learning_to_trivial_lambda0.9_itensorcorrelators_results_") && isdir(name),
        readdir(pwd())
    )
...
function save_plot(summary_df::DataFrame)
    ...
    p = plot(
        ...
        title = L"\lambda_x = 0.63, \lambda_{zz} = 0.07" * " (ITensorCorrelators.jl)",
    ...
    png_file = "learning_to_trivial_lambda0.9_itensorcorrelators_binder_vs_px.png"
    pdf_file = "learning_to_trivial_lambda0.9_itensorcorrelators_binder_vs_px.pdf"
    savefig(p, png_file)
    savefig(p, pdf_file)
...
function main()
    ...
    summary_file = "learning_to_trivial_lambda0.9_itensorcorrelators_analysis_summary.csv"
