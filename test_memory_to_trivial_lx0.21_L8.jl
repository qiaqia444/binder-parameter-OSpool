...
"""
Test Learning-to-Trivial Memory-to-Trivial Family (lambda=0.3)
...
function main()
    println("="^70)
    println("TEST: Memory-to-Trivial Family (lambda=0.3) L=8")
    println("="^70)
    println()
    ...
    # Check 1: Purity should be reduced (dephasing present)
    if result.purity_bar < 1.0
        println("  ✓ Purity reduced by dephasing: $(round(result.purity_bar, digits=6))")
    else
        println("  ⚠ Purity not reduced: $(round(result.purity_bar, digits=6))")
    end
    ...
    # Save results
    output_dir = "test_memory_to_trivial_lx0.21_results"
    mkpath(output_dir)
    
    output_file = joinpath(output_dir, "test_L8_Px0.3.json")
    ...
    if 0.0 <= result.B <= 1.0 && result.n_valid == result.ntrials
        println("="^70)
        println("✓ TEST PASSED - Memory-to-Trivial L=8 works correctly!")
        println("="^70)
        return 0
    else
        println("="^70)
