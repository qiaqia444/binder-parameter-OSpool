...
"""
Test Learning-to-Trivial with L=16, NO dephasing (P_x=0, P_zz=0)

This is a sanity check for a larger system:
- With no dephasing, evolution should be unitary
- Purity should remain 1.0 (pure state)
- Tests that larger systems work correctly
"""
...
function main()
    println("="^70)
    println("TEST: Learning-to-Trivial L=16 with NO DEPHASING (P_x=0, P_zz=0)")
    println("="^70)
    println()
    ...
    # Save results
    output_dir = "test_learning_to_trivial_L16_nodephasing_results"
    mkpath(output_dir)
    
    output_file = joinpath(output_dir, "test_L16_P0.json")
    ...
    if result.purity_bar > 0.99 && 0.0 <= result.B <= 1.0 && result.n_valid == result.ntrials
        println("="^70)
        println("✓ TEST PASSED - L=16 no-dephasing case works correctly!")
        println("="^70)
        return 0
    else
        println("="^70)
